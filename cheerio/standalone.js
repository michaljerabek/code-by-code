...
},{"domhandler":36,"domutils":40}],15:[function(require,module,exports){
"use strict";
var __spreadArray = (this && this.__spreadArray) || function (to, from, pack) {
    if (pack || arguments.length === 2) for (var i = 0, l = from.length, ar; i < l; i++) {
        if (ar || !(i in from)) {
...
function formatAttributes(attributes, opts) {
    ...
    return Object.keys(attributes)
        ...
        if ((attributes[key] === undefined || (value === "" && booleanAttrs.includes(key))) && !opts.xmlMode) {
            return key;
        }
...
function serializeAttributes(node, { treeAdapter }) {
    ...
    for (const attr of treeAdapter.getAttrList(node)) {
        ...
        if (attr.value === ``) {
            ...
            if (location && (location.startOffset + attr.name.length) === location.endOffset) { continue; }
            if (!location && [`allowfullscreen`, `allowpaymentrequest`, `async`, `autofocus`, `autoplay`, `checked`, `controls`, `default`, `defer`, `disabled`, `formnovalidate`, `hidden`, `inert`, `ismap`, `itemscope`, `loop`, `multiple`, `muted`, `nomodule`, `novalidate`, `open`, `playsinline`, `readonly`, `required`, `reversed`, `selected`, `truespeed`].includes(attr.name)) { continue; }
        }
